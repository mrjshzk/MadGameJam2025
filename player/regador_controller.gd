class_name SwayManager extends Node3D
...
func bob(vel : float, delta):
	if vel > 0 and player.is_on_floor():
		...
		position.x = lerpf(position.x, cos(Time.get_ticks_msec() * bob_freq * 0.5) * bob_amount, 10 * delta)
		
	elif not Vector2(position.x, position.y).is_zero_approx():
